...
class FacilityLocationSearchPage extends StatefulWidget{
  ...
  @override
  _FacilityLocationSearchPageState createState() => _FacilityLocationSearchPageState();

}

class _FacilityLocationSearchPageState extends State<FacilityLocationSearchPage>{
      ...
      body: Container(
        child: Column(
          ...
          children: <Widget>[
            ...
            if($facilityLocationRepository.facilityInfoList.length <= 0) introPage(),
            if($facilityLocationRepository.facilityInfoList.length > 0) Expanded(child: getListView(),),
          ...
          children: <Widget>[
            Row(
              ...
              children: <Widget>[
                Container(
                  ...
                  child: isBluetoothConnected?Text('Bluetooth 연결 ', style: TextStyle(color: Colors.black87)):Text('Bluetooth 연결안됨 ', style: TextStyle(color: Colors.black87)),
                ),
                FlatButton(
                  child: Column(
                    children: <Widget>[
                  ...
                  onPressed:()=>{
                    Navigator.pushNamed(context, facilityLocationSearchFilterRoute)
                  },
            ...
            Padding(
              ...
              child: Container(
                ...
                child: Text(filterDisplay(), style: TextStyle(color: Colors.grey),)
              ),
            ),
          ],
        ),
  ...
  String filterDisplay(){
    ...
    if($facilityLocationRepository.searchCondtion != null){
      if($facilityLocationRepository.searchCondtion.facilityCode != ''){
        display = display+ getTranslated(context, 'facility_code') + ' : ' + $facilityLocationRepository.searchCondtion.facilityCode+'    ';
      }

      if($facilityLocationRepository.searchCondtion.assetCode != ''){
        display = display+getTranslated(context, 'input_asset_no') + ' : ' + $facilityLocationRepository.searchCondtion.assetCode+'    ';
      }

      if($facilityLocationRepository.searchCondtion.setupLocationCode != ''){
        display = display+getTranslated(context, 'asset_info_label_setarea') + ' : ' + $facilityLocationRepository.searchCondtion.setupLocationCode+'    ';
      }

      if($facilityLocationRepository.searchCondtion.display != SearchResultDisplay.none){
        if($facilityLocationRepository.searchCondtion.display == SearchResultDisplay.all){
          display = display+getTranslated(context, 'search_result_display') + ' : '+ getTranslated(context, 'all_display')+'    ';
        }else{
          display = display+getTranslated(context, 'search_result_display') + ' : '+ getTranslated(context, 'filter_only')+'    ';
        }
      }
    }
        ...
        child: Column(
          children: <Widget>[
            ...
            Padding(
              ...
              child: Text(getTranslated(context, 'facility_location_search_desc'), style: TextStyle(color: Colors.grey[400]),),
            ),
            Padding(
              padding: EdgeInsets.all(15.0),
              child: Text(getTranslated(context, 'facility_location_search_desc2'), style: TextStyle(color: Colors.grey[400]),),
            ),
          ],
        ),
      ),
  ...
  Widget getListView(){
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index){
        ...
        String searchFacilityCode = $facilityLocationRepository.searchCondtion.facilityCode;
        String searchAssetCode = $facilityLocationRepository.searchCondtion.assetCode;
        String searchSetupLocationCode = $facilityLocationRepository.searchCondtion.setupLocationCode;

        if(searchFacilityCode != ''){
          isCompFacilityCode = $facilityLocationRepository.facilitySearchList[index].facilityCode.toLowerCase().contains(searchFacilityCode.toLowerCase());
        }

        if(searchAssetCode != ''){
          isCompAssetCode = $facilityLocationRepository.facilitySearchList[index].assetCode.toLowerCase().contains(searchAssetCode.toLowerCase());
        }

        if(searchSetupLocationCode != ''){
          isCompSetupLocationCode = $facilityLocationRepository.facilitySearchList[index].setupLocationCode.toLowerCase().contains(searchSetupLocationCode.toLowerCase());
        }

        if(!isCompFacilityCode || !isCompAssetCode || !isCompSetupLocationCode){
                ...
                children: <Widget>[
                  CardSettingsSection(
                    ...
                    header: CardSettingsHeader(
                      label: "[${$facilityLocationRepository.facilitySearchList[index].facilityCode}]" ,
                      showMaterialonIOS: _showMaterialonIOS,
                  ...
                  customCardField(
                    ...
                    content: Text($facilityLocationRepository.facilitySearchList[index].facilityName)
                  ),
                  customCardField(
                    label: getTranslated(context, 'asset_info_label_spec'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].facilitySpec)
                  ),
                  customCardField(
                    label: getTranslated(context, 'asset_info_label_asst_no'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].assetCode, style: TextStyle(color: isCompAssetCode?colorDefaultTextColor:colorSearchDisplayTextColor),)
                  ),
                  customCardField(
                    label: getTranslated(context, 'facility_grade'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].facilityGrade)
                  ),
                  customCardField(
                    label: getTranslated(context, 'plant'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].plantCode+"[${$facilityLocationRepository.facilitySearchList[index].plantName}]")
                  ),
                  customCardField(
                    label: getTranslated(context, 'item_group'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].itemGroup)
                  ),
                  customCardField(
                    label: getTranslated(context, 'asset_info_label_setarea'),
                    content: Text($facilityLocationRepository.facilitySearchList[index].setupLocation, style: TextStyle(color: isCompSetupLocationCode?colorDefaultTextColor:colorSearchDisplayTextColor))
                  ),
  ...
  void clearAll(){
    $facilityLocationRepository.removeAllSearchList();
    ...
    try {
      ...
      scanResult = result;
        if(scanResult.type != ResultType.Cancelled){
          ...
          if(data != null){
            await $facilityLocationRepository.addSearchList(data);
      ...
      if(tagList.length > 0){
        ...
        if(!tempTagList.any((e) => e == data.tagData)){
          ...
          $facilityLocationRepository.getReceiveFacilityList(searchDiv, strScanData, "").then((value){
            if(value != null){
              $facilityLocationRepository.addSearchList(value);
            }
          });
