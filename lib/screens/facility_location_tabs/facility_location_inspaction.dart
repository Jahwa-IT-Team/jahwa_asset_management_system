...
class FacilityLocationInspactionPage extends StatefulWidget {
  ...
  @override
  _FacilityLocationInspactionPageState createState() => new _FacilityLocationInspactionPageState();
}

class _FacilityLocationInspactionPageState extends State<FacilityLocationInspactionPage>{
  ...
  Widget build(BuildContext context){
    ...
    return Container(
      child: Column(children: <Widget>[
        setLocationBox(),
        Expanded(child: getListView($userRepository.connectionInfo.company),) 
      ],) 
    );
  } 
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(children: <Widget>[
          ...
          SizedBox(height: 10,),
          Text('공장을 설정하면 스캔된 설비의 공장이 자동으로 변경됩니다.', style: TextStyle(color: Colors.grey[400])),
          SizedBox(height: 10,)
        ],),
      ),
    );
    ...
    return FutureBuilder(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.none &&
          ...
          return Center(
              child: Text(getTranslated(context, 'empty_value'), style: TextStyle(color: Colors.grey[400]))  //'자산 정보가 존재하지 않습니다.'
          );
