...
class FacilityTradeBluetoothReaderArguments {
  ...
  FacilityTradeBluetoothReaderArguments(
      {@required this.address, @required this.pageType});
}

class FacilityTradeBluetoothReaderPage extends StatefulWidget {
  final FacilityTradeBluetoothReaderArguments screenArguments;
  FacilityTradeBluetoothReaderPage({Key key, @required this.screenArguments})
      : super(key: key);

  @override
  _FacilityTradeBluetoothReaderPageState createState() =>
      _FacilityTradeBluetoothReaderPageState();
}

class _FacilityTradeBluetoothReaderPageState
    extends State<FacilityTradeBluetoothReaderPage> {
  ...
  void initState() {
    ...
    Btprotocol.instance
        .connectDevice(widget.screenArguments.address)
        .then((value) {
      setState(() {
        if (value == 0) {
          connected = 0;
    ...
    Btprotocol.instance.onChangeState.listen((event) {
      dataRead();
    });
    super.initState();
  }
  ...
  void dispose() {
    ...
    if ($facilityTradeRequestRepository != null)
      $facilityTradeRequestRepository.clearRequestScanList(false);
    if ($facilityTradeSendRepository != null)
      $facilityTradeSendRepository.clearSendScanList(false);
    if ($facilityTradeReceiveRepository != null)
      $facilityTradeReceiveRepository.clearReceiveScanList(false);
    ...
    if (connected == 0) {
      ...
      return Scaffold(
        ...
        body: Container(
          child: Column(
            children: <Widget>[
              if (widget.screenArguments.pageType == PageType.Request)
                Expanded(
                  child: getListView1(),
                ),
              if (widget.screenArguments.pageType == PageType.Send)
                Expanded(
                  child: getListView2(),
                ),
              if (widget.screenArguments.pageType == PageType.Receive)
                Expanded(
                  child: getListView3(),
                ),
            ],
  ...
  Widget getListView1() {
    return ListView.builder(
        itemCount:
            $facilityTradeRequestRepository.requestScanRFIDDetailList.length,
        itemBuilder: (context, index) {
          RequestDetail detail =
              $facilityTradeRequestRepository.requestScanRFIDDetailList[index];
          double lastCard = 10.0;
          if ($facilityTradeRequestRepository
                  .requestScanRFIDDetailList.length ==
              (index + 1)) {
            lastCard = 70.0;
          }
          return Card(
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          new Expanded(
                              child: new Text(
                            getTranslated(context, 'asset_info_label_spec'),
                    ...
                    Padding(
                      ...
                      child: Row(
                        ...
                        children: <Widget>[
                          new Expanded(
                              child: new Text(
                            getTranslated(context, 'asset_info_label_asst_no'),
                            style: TextStyle(color: Colors.black87),
                          )),
                          new Expanded(
                              child: new Text(
  ...
  Widget getListView2() {
    return ListView.builder(
        ...
        itemBuilder: (context, index) {
          SendDetail detail =
              $facilityTradeSendRepository.sendScanRFIDDetailList[index];
          double lastCard = 10.0;
          if ($facilityTradeSendRepository.sendScanRFIDDetailList.length ==
              (index + 1)) {
            lastCard = 70.0;
          }
          return Card(
  ...
  Widget getListView3() {
    return ListView.builder(
        itemCount:
            $facilityTradeReceiveRepository.receiveScanRFIDDetailList.length,
        itemBuilder: (context, index) {
          ReceiveDetail detail =
              $facilityTradeReceiveRepository.receiveScanRFIDDetailList[index];
          double lastCard = 10.0;
          if ($facilityTradeReceiveRepository
                  .receiveScanRFIDDetailList.length ==
              (index + 1)) {
            lastCard = 70.0;
          }
          return Card(
  ...
  Widget _buildRequestButton(RequestDetail detail) {
    if (detail.facilityCode != null &&
        !$facilityTradeRequestRepository.requestDetailList
            .any((item) => item.facilityCode == detail.facilityCode)) {
    ...
    } else if ($facilityTradeRequestRepository.requestDetailList
        .any((item) => item.facilityCode == detail.facilityCode)) {
      //이미 등록
      return Padding(
        padding: const EdgeInsets.fromLTRB(10.0, 0.0, 0.0, 0.0),
  ...
  Widget _buildSendButton(SendDetail detail) {
    if (detail.facilityCode != null &&
        !$facilityTradeSendRepository.sendDetailList
            .any((item) => item.facilityCode == detail.facilityCode)) {
      //설비번호가 있고 요청 내역에 없는 스캔 자료
      return Padding(
        padding: const EdgeInsets.fromLTRB(10.0, 0.0, 0.0, 0.0),
    ...
    } else if ($facilityTradeSendRepository.sendDetailList
        .any((item) => item.facilityCode == detail.facilityCode)) {
      //이미 등록
      return Padding(
        padding: const EdgeInsets.fromLTRB(10.0, 0.0, 0.0, 0.0),
  ...
  Widget _buildReceiveButton(ReceiveDetail detail) {
    if (detail.facilityCode != null &&
        !$facilityTradeReceiveRepository.receiveDetailList
            .any((item) => item.facilityCode == detail.facilityCode)) {
      //설비번호가 있고 요청 내역에 없는 스캔 자료
      return Padding(
        padding: const EdgeInsets.fromLTRB(10.0, 0.0, 0.0, 0.0),
    ...
    } else if ($facilityTradeReceiveRepository.receiveDetailList
        .any((item) => item.facilityCode == detail.facilityCode)) {
      //이미 등록
      return Padding(
        padding: const EdgeInsets.fromLTRB(10.0, 0.0, 0.0, 0.0),
      ...
      switch (widget.screenArguments.pageType) {
        case PageType.Request:
          for (SharkDataInfo data in tagList) {
            $facilityTradeRequestRepository.addRequestScanRFIDDetailList(
                RequestDetail(rfid: data.tagData));
          }
          break;
        case PageType.Send:
          for (SharkDataInfo data in tagList) {
            $facilityTradeSendRepository
                .addSendScanRFIDDetailList(SendDetail(rfid: data.tagData));
          }
          break;
        case PageType.Receive:
          for (SharkDataInfo data in tagList) {
            $facilityTradeReceiveRepository.addReceiveScanRFIDDetailList(
                ReceiveDetail(rfid: data.tagData));
          }
          break;
        default:
          break;
