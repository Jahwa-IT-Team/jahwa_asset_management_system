...
class FacilityTradeCommonRepository with ChangeNotifier {
  ...
  List<PickerItem> get itemGroupData {
    ...
    _searchItemGroup.forEach((e) {
      items.add(PickerItem(text: Text("[${e.itemGroupCode}] ${e.itemGroupName}"), value:e.itemGroupCode));
    });
    return items;
  }
  ...
  List<PickerItem> getSetupLocationData(String entCode) {
    ...
    if(_searchSetupLocation.any((e) => e.entCode == entCode)){
      _searchSetupLocation.where((e) => e.entCode == entCode).first.setupLocation.forEach((e) { 
  ...
  String getSetupLocationName(String entCode, String locationCode){
    return _searchSetupLocation.where((e) => e.entCode == entCode).first.setupLocation.where((e) => e.code == locationCode).first.name;
  }
  ...
  //거래처 검색 리스트 변경 이벤트
  Future<void> _onSearchBizPartnerListChange(List<BizPartner> bizPartner) async{
    if(bizPartner == null){
  ...
  //설치 장소 리스트 변경 이벤트
  Future<void> _onSearchSetupLocationListChange(List<SetupLocation> location) async{
    if(location == null){
      ...
      return await http.get(
          ...
          if(response.statusCode == 200){
            ...
            if(_searchLastTime.isBefore(searchTime)){
              _onSearchManagerListChange(responseJson.map((e) => Manager.fromJson(e)).toList());
  ...
  Future<bool> getSearchBizPartnerList(String query) async{
    try{
      ...
      var url = 'https://japi.jahwa.co.kr/api/Facility/GetBizPartnerList?query=$query&langCode=$langCode';
      ...
      return await http.get(
          ...
          if(response.statusCode == 200){
            ...
            //List<Manager> search = [...?_searchManagerList,...?responseJson.map((e) => Manager.fromJson(e)).toList()];
            if(_searchLastTime.isBefore(searchTime)){
              _onSearchBizPartnerListChange(responseJson.map((e) => BizPartner.fromJson(e)).toList());
              _searchLastTime = searchTime;
            }
  ...
  Future<bool> getSearchPlantList() async{
    try{
      ...
      // SERVER LOGIN API URL
      //notifyListeners();
      var url = 'https://japi.jahwa.co.kr/api/Facility/GetSelectList?selectDiv=plant';
      ...
      return await http.get(
          ...
          if(response.statusCode == 200){
            Iterable responseJson = jsonDecode(response.body);
            _onSearchPlantListChange(responseJson.map((e) => Plant.fromJson(e)).toList());
  ...
  Future<bool> getSearchItemGroupList() async{
    try{
      ...
      return await http.get(
          ...
          if(response.statusCode == 200){
            Iterable responseJson = jsonDecode(response.body);
            _onSearchItemGroupListChange(responseJson.map((e) => ItemGroup.fromJson(e)).toList());
  ...
  Future<bool> getSearchSetupLocationList() async{
    try{
      ...
      return await http.get(
          ...
          if(response.statusCode == 200){
            Iterable responseJson = jsonDecode(response.body);
            _onSearchSetupLocationListChange(responseJson.map((e) => SetupLocation.fromJson(e)).toList());
            return true;
          }else{
            return false;
          }
  ...
  Future<RFIDRegInfo> getRFIDFacility(String searchDiv, String value) async{
    try{
      ...
      String langCode = await getLanguageCodeWithCountryCode();
      notifyListeners();
      var url = 'https://japi.jahwa.co.kr/api/Facility/GetFacilitList?searchDiv=$searchDiv&searchText=$value&langCode=$langCode&page=1&pageRowCount=1';
  ...
  //RFID Tag 저장
  Future<bool> setFacilityRFIDTag(String facilityCode, String tagData, String userId) async{
    try{
      /*
      {
